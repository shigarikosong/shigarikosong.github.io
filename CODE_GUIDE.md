...
動画カードは`renderVideoList()`から同ファイル内の生成関数を呼び、再生ボタン、曲名・アーティスト検索、メタ情報、通常タグ、コラボタグを組み立てます。カード項目を変更するときは既存の生成関数へ反映し、`renderVideoList()`の上書きや別経路のカード生成を追加しません。

選択中の検索語・タグ条件は`renderActiveTagChips()`から専用の生成関数を呼んで表示し、解除処理は`clearActiveFilterChip()`へ集約しています。個別チップのために親要素へ別のクリック監視を追加しません。
