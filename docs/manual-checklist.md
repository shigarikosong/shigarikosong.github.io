# Manual Checklist

PR前やPreview Deployment確認時に使う手動チェックリストです。

変更範囲に関係する項目を中心に確認してください。タグ・フィルター・再生まわりを触った場合は、該当セクションをなるべく一通り確認します。

`pnpm test`と`pnpm run test:regressions`には、フィルター一括更新の矛盾防止、終了時刻の判断、実際の画面コードを使う連携テストが含まれます。連携テストは`index.html`の読み込み順でデータ処理・プレイヤー・PC/モバイルのフィルター処理を実行します。通信と動画プレイヤーは模擬するため、実際のYouTube/TikTok再生、見た目、スクロール位置は引き続きPreviewで確認してください。

テスト環境はNode.js 24.15以降の24系を使用します。画面DOMの検査に使う`jsdom`は開発依存で、公開ページには読み込みません。

`scripts/accessibility.test.mjs`では、タグ再描画後の操作位置、選択状態の読み上げ用情報、モーダルの開閉処理も検査します。テスト内のdialog APIと画面幅ごとの表示は模擬です。ブラウザ標準の背景操作の抑止、実際のTab移動、読み上げソフトとの連携はPreviewで確認してください。

## 1. Basic Display

- [ ] Tailwind CDNへのリクエストが発生せず、`tailwind.generated.css`が読み込まれる。
- [ ] 通常タグと絞り込み・ランダム再生などの通常操作がグレー基調になり、タグ種別ごとの多色表示が残っていない。
- [ ] 選択中タグは青、除外中タグは赤、再生中カードは黄色で識別できる。
- [ ] PC・モバイル・モーダルの件数表示がグレー基調で統一されている。
- [ ] 初期表示で動画一覧が読み込まれる。
- [ ] 初期表示で全件表示になっている。
- [ ] 件数表示が `全 n 件中 m 件表示` の形で崩れていない。
- [ ] ブラウザのコンソールに新しいエラーが出ていない。
- [ ] PC幅で大きなレイアウト崩れがない。
- [ ] スマホ幅で大きなレイアウト崩れがない。

## 2. Search And Sort

- [ ] 曲名またはアーティスト名で検索できる。
- [ ] スペース区切りのAND検索が動く。
- [ ] `OR` 検索が動く。
- [ ] `-除外語` 検索が動く。
- [ ] 新しい順、古い順、タイトル順、アーティスト順が切り替わる。
- [ ] 検索や並び替え後に件数表示が更新される。
- [ ] PC検索欄のクリアボタンで検索語だけ解除でき、タグ条件と並び順は維持される。
- [ ] スマホ幅では絞り込み／ランダム再生の下に検索欄が表示され、ページをスクロールしても上部に残る。
- [ ] スマホ検索欄のクリアボタンで検索語だけ解除でき、タグ条件と並び順は維持される。
- [ ] PC・スマホ検索欄の内容が同期し、画面幅を切り替えても検索語が維持される。
- [ ] PC・スマホで連続入力すると、入力を止めるまで待たずに検索結果と条件チップが反映される。入力後に検索欄から離れたりクリアを押したりしても、同じ検索の二重処理による停止が増えていない。
- [ ] 検索結果の曲と順序が変わらない入力でも、検索語・チップ・クリアボタンが更新され、再生中表示とキーボードの操作位置が維持される。
- [ ] 表示件数が変わらないタグ選択・解除でも、カード内タグの色と読み上げ用の選択状態が更新される。
- [ ] アクティブチップがある場合は、スマホ検索欄より下に表示される。
- [ ] リセット後、検索語と並び順が初期状態に戻る。

## 3. Tag Filters

- [ ] Style / category タグで絞り込みできる。
- [ ] Platform タグで絞り込みできる。
- [ ] Time タグで絞り込みできる。
- [ ] Time タグの選択状態とアクティブタグチップが一致する。
- [ ] Format タグで絞り込みできる。
- [ ] `3D` と `Shorts` がFormat UI上で従来通り動く。
- [ ] Riko Part / role タグで絞り込みできる。
- [ ] Riko Partのincludeタグを複数選ぶと、Riko Part内はOR条件で絞り込まれる。
- [ ] Riko Partのinclude複数選択と他グループの条件を組み合わせると、グループ間はAND条件になる。
- [ ] Collab タグで絞り込みできる。
- [ ] Collabのincludeタグを複数選ぶと、Collab内はOR条件で絞り込まれる。
- [ ] Collabのinclude複数選択と他グループの条件を組み合わせると、グループ間はAND条件になる。
- [ ] 複数タグの組み合わせで絞り込みできる。
- [ ] タグと検索語を組み合わせても絞り込みできる。
- [ ] タグ解除後に件数と一覧が戻る。

## 4. Include / Exclude Tags

- [ ] 対象タグが `none -> include -> exclude -> none` の順で切り替わる。
- [ ] `include` と `exclude` が同じタグで同時に有効にならない。
- [ ] `include` 条件がアクティブタグチップに表示される。
- [ ] Riko Partのinclude条件を複数選ぶと、アクティブタグチップも複数表示される。
- [ ] Collabのinclude条件を複数選ぶと、アクティブタグチップも複数表示される。
- [ ] `exclude` 条件が `- Shorts` のような除外チップで表示される。
- [ ] Riko Partのexclude条件を複数選ぶと、どれか1つでも含む動画が除外される。
- [ ] Collabのexclude条件を複数選ぶと、どれか1つでも含む動画が除外される。
- [ ] アクティブタグチップを押すと、その `include` 条件だけ解除される。
- [ ] 除外チップを押すと、その `exclude` 条件だけ解除される。
- [ ] リセットで `include` と `exclude` の両方が解除される。
- [ ] 除外条件の追加・解除後、PC/モバイル/リスト内タグの赤い除外表示が一致する。
- [ ] include / exclude表示は各UIのタグ描画時に `FilterTagView` から反映され、後追いの全ボタン探索を必要としない。
- [ ] 除外表示の `- ` が付いてもカテゴリ・Collabタグの表示順が変わらない。

## 5. Desktop Filter Panel

- [ ] PC幅で「絞り込み」パネルを開閉できる。
- [ ] PC幅で件数表示が更新される。
- [ ] PCフィルター内のSortが動く。
- [ ] PCフィルター内の各タグが動く。
- [ ] PCフィルター内のタグ状態がアクティブチップと一致する。
- [ ] PCフィルターの「閉じる」後、再生中カードが表示リスト内にあればそこへ、なければ結果件数または動画一覧の先頭付近へ移動する。
- [ ] PC表示で不自然なスクロールジャンプが増えていない。

## 6. Mobile Filter Modal

- [ ] スマホ幅でフィルターモーダルを開閉できる。
- [ ] モーダル内Sortが動く。
- [ ] モーダル内の各タグが選択・解除できる。
- [ ] モーダル内リセットで条件が解除される。
- [ ] モーダル内リセット後、タグ表示が先に反応し、一覧と件数が一度だけ更新される。
- [ ] モーダルの「閉じる」は現在の条件を維持する。
- [ ] タグ・検索を即時反映した後の「閉じる」で、不要な一覧再描画による長い停止が発生しない。
- [ ] モーダルの「閉じる」後、再生中カードが表示リスト内にあればそこへ、なければ結果件数または動画一覧の先頭付近へ移動する。
- [ ] ページ下部でモーダルを開いて件数が減るタグを選び、閉じても下部に取り残されない。

## 7. Video List Tag Clicks

- [ ] リスト内カードのStyle / Platform / Format / Role / Collabタグを押すと絞り込みできる。
- [ ] リスト内カードの再生ボタンで動画を再生できる。
- [ ] リスト内カードの曲名クリックで、その曲名に検索条件が置き換わる。
- [ ] リスト内カードのアーティスト名クリックで、そのアーティスト名に検索条件が置き換わる。
- [ ] 曲名・アーティスト名クリックでは動画が誤再生されない。
- [ ] コラボユニットがあるカードは人数に関係なく、ユニット名と `+N` を表示し、個別メンバーを初期状態で収納する。
- [ ] コラボユニットがないカードは、コラボライバー4人以下なら全員を表示し、5人以上なら一部表示せず `+N` のみにする。
- [ ] コラボメンバーを展開すると全員が表示され、トグルが `-` に変わり、再収納できる。
- [ ] コラボメンバーの `+N` / `-` は透明背景・薄いグレー枠の補助操作として表示され、通常タグより目立たない。
- [ ] コラボメンバーの `+N` / `-` はhover・キーボードフォーカス時に操作可能だと判別できる。
- [ ] 展開したコラボライバータグから従来どおり絞り込みできる。
- [ ] 短い曲名と長いアーティスト名など、長さの偏りがあるカードでも従来どおり自然に幅配分される。
- [ ] 長い曲名・アーティスト名の横スクロールがPC・モバイルで動き、画面幅変更後も再判定される。
- [ ] 再生ボタンクリックでは検索・タグ絞り込みが誤発動しない。
- [ ] リスト内タグに `data-filter-group` と `data-filter-value` が付いている。
- [ ] リスト内タグクリック後、元カードが残る場合はそのカード付近へ戻る。
- [ ] 元カードが非表示になる場合は、noticeまたはリスト上部へ移動する。
- [ ] リスト内タグの選択状態がPC/モバイルフィルターと矛盾しない。
- [ ] アクティブチップが増減した後も、フィルタ後スクロール先が中途半端な位置にならない。

### Number Inspection

- [ ] 通常時はカードの番号・コピー操作・番号表示終了ボタンが見えない。
- [ ] 「このサイトについて」の見出しを続けて5回クリック／タップすると番号表示が有効になり、PCではEnter／Spaceでも有効にできる。
- [ ] 同じ曲・同じ動画の別区間でも、それぞれのJSONの`number`が表示される。並び順や検索を変えても番号が入れ替わらず、重複表示されない。
- [ ] 番号表示の切り替えで検索・タグ・再生が変わらず、展開したコラボ欄と閲覧位置が維持される。
- [ ] PC・スマホで番号とコピーアイコンが収まり、長い枠名や固定プレイヤーと不自然に重ならない。
- [ ] コピーアイコンを押すと番号だけがコピーされ、検索や動画再生は発生しない。コピー許可がない場合は成功表示にならず、番号を手動でコピーできる。
- [ ] 「番号表示を終了」で元のカード表示に戻り、終了ボタンに操作位置があった場合は見出しへ戻る。ページを再読み込みしても通常表示に戻る。

### Shared Filter URLs

- [ ] 初期状態では共有アイコンが見えず、検索・選択・除外・並び順を変更するとPCとスマホの一覧件数の隣に表示される。0件でも共有でき、リセットすると消える。
- [ ] 件数と共有アイコンが同じ行に収まり、狭いPC幅・スマホ幅でも行の追加や検索欄・固定UIとの重なりがない。
- [ ] コピーしても検索・再生・スクロール・アドレス欄が変わらず、成功時だけコピー済みの通知が出る。
- [ ] 別タブで共有URLを開くと、検索文・選択／除外タグ・期間・並び順が再現され、自動再生は始まらない。PCとスマホで同じ条件になる。
- [ ] 共有後のタグ修正や動画追加は、共有先にも最新データとして反映される。PreviewでコピーしたURLはPreviewを指す。
- [ ] コピー拒否時は選択済みのURLを手動コピーできる。Tabがダイアログ内を循環し、Escape・閉じる・背景クリックで元の操作位置へ戻る。
- [ ] 同じページで別の共有URLへ移動・戻る／進むを行うと条件全体が置き換わり、古い条件やモバイルの保留中並び順が混ざらない。再生中なら再生は継続する。
- [ ] 不正な共有URLでは通知と通常の一覧が表示され、削除済みのタグは条件チップに残る。受け取ったURLの条件を編集してもアドレス欄は変わらず、再読み込みでは受け取った条件に戻る。
- [ ] PC・スマホでサイトタイトルを押すと、クエリ・共有条件のない同じサイトのトップへ再読み込みされ、検索・タグ・並び順が初期化される。URLに条件がない状態でも同じ動作になり、再読み込み後も条件は復活しない。
- [ ] タイトルへのTab移動ではフォーカス枠が見え、Enterでもトップへ戻れる。新しいタブで開く操作は通常どおり使え、ヘッダーの改行・高さは変わらない。
- [ ] タイトルから戻ると再生が止まり、保存済みのプレイヤーサイズ・位置は維持される。PreviewではPreviewのトップへ戻り、ブラウザの「戻る」で元の共有URLへ戻れる。

## 8. Playback

- [ ] カード左側の再生ボタンクリックで固定プレイヤーが表示される。
- [ ] 通常時はRepeat OFFでもYouTube再生ボタンクリックで従来どおり再生開始する。
- [ ] infoモーダル内の「視聴履歴について」に手動再生モードの説明と切り替えボタンが表示される。
- [ ] infoモーダル内のボタン、`?manualPlay=1`、または `Ctrl + Alt/Option + Shift + M` で検証モードをONにすると、YouTube再生ボタンクリックでは動画が表示され、YouTube画面内の再生ボタンを押すまで再生開始しない。
- [ ] infoモーダル内のボタン、`?manualPlay=0`、または `Ctrl + Alt/Option + Shift + M` で検証モードをOFFに戻せる。
- [ ] infoモーダル内のボタン文言が現在のON/OFF状態に合わせて更新される。
- [ ] YouTube動画が再生できる。
- [ ] 一覧表示直後にYouTubeの再生ボタンを押してもエラーにならず、プレイヤー準備後に選択した動画が再生される。
- [ ] YouTube準備中に別のYouTube動画を選ぶと、最後に選んだ動画だけが再生される。
- [ ] YouTube準備中にTikTokへ切り替えるかプレイヤーを閉じると、待機中のYouTube動画があとから再生されない。
- [ ] `script.js`がYouTube準備待ちを担当し、削除済みの`youtube-stability.js`への読み込み・参照が残っていない。
- [ ] 手動再生モード中は、Repeat ONE / Repeat ALL / RandomによるYouTube遷移でも動画をcueし、YouTube画面内の再生ボタンを押すまで再生開始しない。
- [ ] 固定プレイヤーの再生／一時停止ボタンでYouTubeを操作でき、アイコンとラベルが切り替わる。
- [ ] YouTube画面内で再生／一時停止した場合も、固定プレイヤー側のアイコンが追従する。
- [ ] 再生要求後の `BUFFERING` / `UNSTARTED` 中に、一時停止アイコンが再生アイコンへ戻らない。
- [ ] プレイヤー収納中も固定プレイヤーの再生／一時停止ボタンを操作できる。
- [ ] YouTube の開始秒 `start` が反映される。
- [ ] `end`を越える位置へシークした動画から次の歌枠行へ進んでも、切替直後の古い再生時刻によって次の行が即スキップされない。
- [ ] YouTube標準の動画終了と`end`指定カウントダウンが同じRepeat / Random遷移ルールを使う。
- [ ] TikTok埋め込みが表示される。
- [ ] TikTokは読み込み直後に自動再生されず、埋め込み画面内から最初の再生を開始できる。
- [ ] TikTok初回再生前は固定プレイヤーの再生ボタンが半透明の無効状態になり、追加の案内文は表示されない。
- [ ] TikTokを画面内から一度再生した後は、固定プレイヤーの再生／一時停止ボタンで操作できる。
- [ ] TikTok画面内で再生／一時停止した場合も、固定プレイヤー側のアイコンが追従する。
- [ ] TikTok初回再生前はアイコンだけ一時停止表示へ先走らず、画面内から再生後に一時停止表示へ切り替わる。
- [ ] 再生中カードに `playing` 表示が付く。
- [ ] 絞り込み後も再生中カードが表示対象に残る場合、再生中カードへ移動できる。
- [ ] 再生中カードが非表示になる場合、filtered-out noticeが表示される。
- [ ] プレイヤーを閉じると再生中表示が解除される。
- [ ] プレイヤーの開閉・収納・高さ変更が壊れていない。
- [ ] 通常YouTubeは高さ変更時も16:9を維持し、外側に横幅いっぱいの黒背景が残らない。
- [ ] PC・モバイルとも通常YouTubeを約356×200まで縮小でき、16:9を維持する。
- [ ] 通常YouTubeを356×200よりさらに縮小すると、高さ200pxを維持したまま幅が縮まり、約200×200まで小さくできる。
- [ ] 横動画の200×200付近のコンパクトサイズが、再読み込みや動画切替後も維持される。
- [ ] YouTube Shortsは9:16を基準に表示され、利用可能な場合は200px未満の幅にならない。
- [ ] TikTokは公式Embed Player iframeで表示され、9:16を基準に画面内へ収まる。
- [ ] PC・モバイルともShorts／TikTokを200×200付近まで縮小でき、コンパクト時だけ9:16より横長になる。
- [ ] 縦型プレイヤーを小さくしたまま画面幅を狭幅・幅広へ切り替えても、大きさがリセットされない。
- [ ] 縦型プレイヤー周囲の透明部分が、背後に見えている一覧カードの再生ボタンを遮らない。
- [ ] ShortsではないYouTube行でも、`player_aspect` が `9:16` なら縦型で表示される。
- [ ] ShortsのYouTube行でも、`player_aspect` が `16:9` なら横型で表示される。
- [ ] 低い画面やスマートフォン横向きでもプレイヤーが画面外へはみ出さない。
- [ ] リサイズハンドルが実際の映像幅に追従する。
- [ ] 見えるハンドルがプレイヤー上辺の左寄りにあり、PCでは上辺全体から同じドラッグ操作を開始できる。
- [ ] ハンドルを上下へドラッグすると従来どおり縦横比を維持してサイズ変更できる。
- [ ] ハンドルを左右へドラッグするとサイズを維持したままプレイヤーだけ水平移動する。
- [ ] ハンドルを横へ動かしたまま上下にも動かすと、持ち直さずに水平移動とサイズ変更を同時操作できる。
- [ ] PC・モバイルともドラッグ中の表示が指やポインターへ滑らかに追従する。
- [ ] 縦・横だけを意図した操作では、小さな直交方向の指ぶれによって位置やサイズが変化しない。
- [ ] ハンドル操作中だけ左右矢印が表示され、操作終了後に消える。
- [ ] モバイルで小さくしたプレイヤーのハンドルへ一瞬触れても、現在の大きさがリセットされない。
- [ ] モバイルのハンドルは小さな指ブレでは動かず、意図して動かした場合は約8pxで追従を開始する。
- [ ] モバイルでは透明な操作範囲が上辺全体まで広がらず、ハンドル周辺以外のスクロール操作を奪わない。
- [ ] 水平位置の保存値がない初回表示ではプレイヤーが右端寄せになり、初期高さは従来どおり360pxを基準にする。
- [ ] 水平位置が再読み込み後も維持され、動画の縦横切替・画面回転後も画面外へ出ない。
- [ ] 展開中の小さくする／閉じる／カウントダウン／Full ver.操作が映像枠右上に揃い、iframeを覆わない。
- [ ] プレイヤーを開いた直後は収納／閉じる操作が表示され、約2〜3秒の無操作後にフェードアウトする。
- [ ] PCでは動画面・上部ハンドル・操作付近へのhoverで、収納／閉じる操作が再表示され、動画面を離れるまで表示が維持される。
- [ ] モバイルではハンドル操作、Now Playing部分のタップ、YouTube／TikTokの再生状態変化によって収納／閉じる操作が再表示される。
- [ ] モバイルの動画iframe上に透明なタップ領域がなく、最初の再生・一時停止・シーク操作を奪わない。
- [ ] キーボードでプレイヤー操作へ移動している間は収納／閉じる操作が消えず、フォーカス可能である。
- [ ] 収納／閉じる操作の非表示中は透明なクリック領域が残らない。
- [ ] カウントダウン／Full ver.は収納／閉じる操作と独立して表示され、それだけを理由に収納／閉じる操作が再表示されない。
- [ ] 収納／閉じる操作が非表示の状態でも、カウントダウン／Full ver.を1回のタップで操作でき、押下途中に位置がずれない。
- [ ] プレイヤー幅300px以下では通知操作が上段、表示中の収納／閉じる操作が下段になり、非表示時は空白行が残らない。
- [ ] 細い縦型プレイヤーでも長い右上操作が画面外へ出ず、クリックできる。
- [ ] 画面幅変更・回転後に比率と位置が再計算され、再生／一時停止状態は変わらない。
- [ ] 収納中は開く／閉じる操作が常時表示され、復元後は保存済みの希望サイズを基準に表示される。
- [ ] リサイズ中もendカウントダウンとFull ver.表示タイミングが継続する。
- [ ] `full_number` が空欄、または誘導先 `number` が存在しない行ではFull ver.誘導ボタンが表示されない。
- [ ] `full_number` の誘導先が存在するYouTube行では、終了10秒前にFull ver.誘導ボタンが表示される。
- [ ] `full_number` の誘導先が存在するTikTok行では、再生中にFull ver.誘導ボタンが表示される。
- [ ] `full_button_text` がある行では、その文言がFull ver.誘導ボタンに表示される。
- [ ] `full_button_text` が空欄の行では、`Full ver. を再生` が表示される。
- [ ] Full ver.誘導ボタンを押すと、誘導先行の `start` / `end` を使って再生される。
- [ ] 誘導先が現在の絞り込みリスト外の場合、filtered-out notice付近へ移動する。
- [ ] 再生動画切替・プレイヤー終了時にFull ver.誘導ボタンが消える。

## 9. Random / Previous / Next / Repeat

- [ ] ランダム再生できる。
- [ ] 絞り込み後のランダム再生対象が現在の表示リストになっている。
- [ ] 除外条件適用後のランダム再生対象が現在の表示リストになっている。
- [ ] 除外条件適用後、Next / Previous とランダム再生が非表示カードを選ばない。
- [ ] Next / Previous が現在の表示リストに沿って動く。
- [ ] 通常時はRepeat OFFでもYouTube Next / Previous / ランダム再生が従来どおり再生開始する。
- [ ] Random ON のPreviousは直前に再生していた曲へ戻る。
- [ ] Random OFF のNextは順番通りに動く。
- [ ] Random ON のNextはランダムキューに沿って動く。
- [ ] 検証モードONのYouTube Next / Previous / ランダム再生は動画を表示するが、YouTube画面内の再生ボタンを押すまで再生開始しない。
- [ ] 検証モードONで全曲リピートの動画終了後に次動画へ遷移しても、自動再生せずYouTube画面内の再生ボタンを待つ。
- [ ] 全曲リピート + Random OFFの自動連続再生でもTikTokを飛ばし、次のYouTube動画へ進む。
- [ ] Repeat OFF / 1曲リピート / 全曲リピートの表示と動作が大きく崩れていない。
- [ ] リピートOFFとシャッフルOFFは半透明、ON状態は不透過で表示される。
- [ ] 操作ボタンが「シャッフル、前へ、再生／一時停止、次へ、リピート」の順でPC・スマホとも一列に収まる。
- [ ] 全曲リピート + Random ON でTikTokが自動連続再生対象から外れる。

## 10. Keyboard And Scroll

- [ ] PC・スマホ幅のタグをTabで選び、Enter／Spaceを続けて押すと「選択→除外→解除」でき、同じタグに操作位置が残る。
- [ ] 並び順をキーボードで変更しても操作位置が残り、読み上げで現在の選択肢が分かる。
- [ ] タグは読み上げで「選択中」「除外中」を区別でき、解除後に古い状態が残らない。
- [ ] カードの曲名・アーティスト・タグで絞り込んでも同じカードの操作位置が維持され、元のカードが消えた場合は動画一覧へ移る。
- [ ] 条件チップの解除後、残るチップへ操作位置が移り、最後のチップを解除すると動画一覧へ移る。
- [ ] スマホの絞り込みを開くとモーダル内へ操作位置が移り、Tab／Shift+Tabで背後の検索・再生ボタンへ抜けない。
- [ ] スマホの絞り込みはEscapeでも閉じられ、選んだ条件を維持して開くボタンへ操作位置が戻る。
- [ ] 並び順変更・リセット直後にEscapeで閉じても変更が一度だけ反映され、スクロール禁止が残らない。
- [ ] 絞り込みを閉じてすぐ開き直しても、遅れて背後の一覧へスクロールしない。
- [ ] モーダルを開いたまま画面を回転・リサイズしても、閉じた後の操作位置が非表示ボタンへ戻らない。
- [ ] 「このサイトについて」はTab／Shift+Tabが内部で循環し、Escape・上下の閉じるボタン・背景クリックで閉じて元の操作位置へ戻る。
- [ ] PC絞り込みパネル内でEscapeを押すと閉じ、絞り込みボタンへ操作位置が戻る。
- [ ] モーダル内でShift+A／Shift+Dを押しても、背後の曲が切り替わらない。
- [ ] `Shift + A` で前の曲へ移動する。
- [ ] `Shift + D` で次の曲へ移動する。
- [ ] 入力欄、textarea、select、contenteditableにフォーカス中はショートカットが発火しない。
- [ ] 再生中カードへ戻る `♪` ボタンが必要な時だけ表示される。
- [ ] 固定プレイヤーやstickyフィルターに、移動先のカードやnoticeが隠れない。
- [ ] 再生開始・フィルター後・`♪` ボタンからの移動で、再生中カードへのスクロール位置が従来どおりになる。

## 11. Data And Tag Definition Changes

- [ ] タグ定義や表示順を変えた場合、`tag-config.js` が正本になっている。
- [ ] 新しいタグボタンに `data-filter-group` と `data-filter-value` が付いている。
- [ ] Platform内部値は lowercase の `youtube` / `tiktok` のままになっている。
- [ ] Time内部値は `recent` / `year` / `old` のままになっている。
- [ ] スプレッドシート列名を変えていない、または参照コードも合わせて更新している。

## 12. Script Loading And Helper Boundaries

- [ ] `index.html` の読み込み順で `tag-config.js` / `date-utils.js` / `video-normalizer.js` / `search-utils.js` / `video-query.js` / `filter-state.js` / `filter-tag-view.js` / `focus-utils.js` / `playback-policy.js` / `playback-transition-policy.js` / `end-countdown-policy.js` が、依存するスクリプトより前にある。
- [ ] タグ系補助スクリプトは `index.html` で明示読み込みされ、`loading-status.js` から後追い読み込みされていない。
- [ ] `loading-status.js` が `window.fetch`、`populateFilters()`、`renderVideoList()`、`loadVideo()`を上書きしていない。
- [ ] 動画JSONの取得・配列確認・必須項目確認が`script.js`の専用読み込み処理で行われる。
- [ ] 動画JSONの取得失敗・不正形式時に、0件一覧へ進まず読み込みエラー表示が残る。
- [ ] `ui-polish.js` が `renderVideoList()` を上書きしていない。
- [ ] タグ状態が`FilterState`内で保持され、`selectedCategoryTag`などの古いグローバル変数が復活していない。
- [ ] 枠名とコラボメンバー収納が `script.js` のカード描画内で反映され、削除済みの `waku-name-display.js` への参照が残っていない。
- [ ] 削除済みの `exclusion-style-sync.js` への読み込み・参照が残っていない。
- [ ] 削除済みの `filter-active-style-sync.js` への読み込み・参照が残っていない。
- [ ] ブラウザ標準の `Element.prototype.scrollIntoView` を上書きするコードが残っていない。
- [ ] `index.html` と実行用JavaScriptに、削除済みの `playing-scroll-position.js` への読み込み・参照が残っていない。
- [ ] 旧 `tag-exclusion.js` への読み込み・参照が残っていない。
- [ ] `time-tag-active.js` など削除済み補助スクリプトへの参照が残っていない。

## 13. Pre-Merge Diff Check

- [ ] `pnpm run validate:data`が成功する。
- [ ] `pnpm test`が成功する。
- [ ] `git status` で意図したファイルだけが変更されている。
- [ ] `.DS_Store` が差分に含まれていない。
- [ ] `git diff` に見た目変更や大規模差分が混ざっていない。
- [ ] 挙動変更とリファクタリングが不要に混ざっていない。
- [ ] 変更範囲に応じて `README.md`、`AGENTS.md`、`docs/filter-tag-rules.md`、`docs/player-behavior-rules.md` の更新要否を確認した。

## 14. Preview Deployment

- [ ] Cloudflare Pages のPreview Deploymentで確認した。
- [ ] Preview URLで動画一覧が読み込まれる。
- [ ] Preview URLで検索・タグ・再生の主要導線が動く。
- [ ] 本番URLで確認すべき差分がある場合、マージ後の確認項目をPRにメモした。
