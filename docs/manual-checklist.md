...
- [ ] TikTok埋め込みが表示される。
- [ ] TikTokプレイヤー準備後、固定プレイヤーの再生／一時停止ボタンで操作できる。
- [ ] TikTok画面内で再生／一時停止した場合も、固定プレイヤー側のアイコンが追従する。
- [ ] TikTok iframeの`load`後は、`onPlayerReady`が届かない環境でも中央ボタンが有効になり、再生／一時停止命令が状態変化まで短時間再送される。
- [ ] 初回再生がブラウザに拒否された場合、アイコンだけ一時停止表示へ先走らず、TikTok画面内から再生後は中央ボタンで操作できる。
...
- [ ] モバイルでハンドル周辺をスクロールするとき、見えているグリップ外の透明領域が操作を奪わない。
