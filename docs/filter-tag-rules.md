...
- The header site title links to `./` on the current host, without a query or fragment. Normal activation reloads the default list at the top, including when only the fragment differs or the URL is already clean. Playback stops and saved player preferences remain untouched. `site-home-link.js` handles same-document reloads; modified clicks and opening in a new tab retain native link behavior. Returning from a shared URL keeps that URL available through browser Back.
